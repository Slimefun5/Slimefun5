...
public class ExpCollector extends SlimefunItem implements InventoryBlock, EnergyNetComponent {
    ...
    protected void constructMenu(BlockMenuPreset preset) {
        for (int slot : border) {
            preset.addItem(slot, CustomItemStack.create(XMaterial.PURPLE_STAINED_GLASS_PANE.parseMaterial(), " "), (p, s, item, action) -> false);
        }
    }
    ...
    protected void tick(Block block) {
        ...
        Iterator<Entity> iterator = EntityCompat.getNearbyEntities(block.getWorld(), location, range, range, range, n -> n instanceof ExperienceOrb && n.isValid()).iterator();
