...
public class AutoBreeder extends SlimefunItem implements InventoryBlock, EnergyNetComponent {
    ...
    protected void tick(Block b) {
        ...
        for (Entity n : EntityCompat.getNearbyEntities(b.getWorld(), b.getLocation(), 4.0, 2.0, 4.0, this::canBreed)) {
            for (int slot : getInputSlots()) {
                if (SlimefunUtils.isItemSimilar(inv.getItemInSlot(slot), organicFood, false)) {
                    ...
                    ReflectionCompat.invoke((Animals) n, "setLoveModeTicks", 600);
                    ParticleCompat.spawn(n.getWorld(), Particle.HEART, ((LivingEntity) n).getEyeLocation(), 8, 0.2F, 0.2F, 0.2F);
                    return;
                }
            }
        }
    ...
    private boolean canBreed(@Nonnull Entity n) {
        if (n.isValid() && n instanceof Animals) {
            Animals animal = (Animals) n;            return animal.isAdult() && animal.canBreed() && !Boolean.TRUE.equals(ReflectionCompat.invoke(animal, "isLoveMode"));
        }
