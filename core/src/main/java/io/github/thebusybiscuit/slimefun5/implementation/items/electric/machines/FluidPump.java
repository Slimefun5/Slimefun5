...
public class FluidPump extends SimpleSlimefunItem<BlockTicker> implements InventoryBlock, EnergyNetComponent {
    ...
    private Block findNextFluid(@Nonnull Block fluid) {
        if (fluid.getType() == Material.WATER || fluid.getType() == XMaterial.BUBBLE_COLUMN.parseMaterial()) {
            /**
    ...
    private @Nonnull ItemStack getFilledBottle(@Nonnull Block fluid) {
        // BUBBLE_COLUMN (1.13+) is absent on the 1.8.8 enum, so compare against XMaterial-resolved
        // constants instead of a switch. WATER and BUBBLE_COLUMN shared a fall-through body originally.
        Material type = fluid.getType();
        if (type == XMaterial.WATER.parseMaterial() || type == XMaterial.BUBBLE_COLUMN.parseMaterial()) {
            ItemStack waterBottle = new ItemStack(Material.POTION);
            PotionMeta meta = (PotionMeta) waterBottle.getItemMeta();
            if (Slimefun.getMinecraftVersion().isBefore(20, 2)) {
                PotionCompat.setBasePotionData(meta, new PotionData(PotionType.WATER));
            } else {
                PotionCompat.setBasePotionType(meta, PotionType.WATER);
            }
    ...
    private @Nonnull ItemStack getFilledBucket(@Nonnull Block fluid) {
        ...
        } else if (type == XMaterial.WATER.parseMaterial() || type == XMaterial.BUBBLE_COLUMN.parseMaterial()) {
